...
NORMALIZED_PATH = 'normalized/split' # when we count how many times the author has cited an article
UNORMALIZED_PATH = 'unormalized/split' # when we doesn't cara about how many times the author has cited an article
...
# splitting
train, test = data.random_split(.75, seed=9)
print len(train), len(test)

# aggregates and saves the files
grouped_citations = train.groupby(
    key_columns='author',
    operations={'ratings': agg.CONCAT('reference', 'score')},
)
